...
再退 4，因为那正是自旋存在的理由。判定每次调用只做一次而不是每轮做一次 —— 那个测试是一次 fork，
而一把在 i=0 时还新鲜的锁，不可能在 5s 的自旋窗口里老过一分钟。抢锁**输给**另一个抢锁者时
退回自旋，而不是就地失败：赢家手里握着的是一把真锁，那恰好就是自旋要等的东西。
...
不在成功路径上赋值 —— 否则队列里第一条带偏移、第二条不带时，全局会留着上一条的值）；
`--start SEC` 是调用方这一次的覆盖。`run_mpv` 里合成一次：

```
   local start_at="${PLAY_START:-$RESOLVED_START}"      # flag 压过引擎，与套件的四层链同向
   [[ -n "$start_at" ]] && mpv_cmd+=(--start="$start_at")
```

用 `:-` 而不是"非空判断"是有意的：`--start 0`（"无视这条链接里的 `t=`，从头放"）
是一个真实的意图，必须压过引擎给的值，而不是掉回去。同理，`RESOLVED_START` 读信封时用的是
**类型判断**而不是 `// ""` —— `?t=0` 给的 `0` 是一个真答案，`// ""` 会把它和 `null` 一起丢掉。

`--start=` 放在 `--volume=` 旁边，而不是放进那批模式选项：那批之所以在最后，
是因为 mpv 末位优先、而它们是在覆盖前面的东西；这一个不覆盖任何人。

**队列作用域：只作用于第一条。** 它是一次性的启动意图（"从这儿接着听"），
不是队列级策略 —— "每首歌都跳过前十分钟"没有真实用例。播放循环在第一次
`play_url_directly` 之后就把 `PLAY_START` 清空，且清在 `case` **之前**：
一次 `--next` 跳过也要花掉它，因为调用方已经离开了那条偏移所指的曲目。
