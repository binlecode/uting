...
- **D18 —— 配置是一条四层链；出厂默认值是一个被跟踪的数据文件，且按 DATA 读、绝不 source。**
  ...
  **出厂默认值住在 `<checkout>/config`**，每个入口点从自己**解析后**位置的上一级读它 ——
  ...
  用户那份可以不存在。**出厂那份永不被任何命令写入；用户那份由 `uting` 写回六个键** ——
  见下面「写回」。
  ...
  **写回 —— 本条的一半在 2026-08-29 被反转。** 原文是"两份都不由任何命令写入 ——
  这套东西不持久化配置"。对**出厂那份**仍然成立（它是这个 checkout 声明的默认值，
  不是任何人的偏好）；对**用户那份**不再成立：`uting` 把它在运行时改的六个键
  （`UT_DEFAULT_ENGINE` `YT_THEME` `UT_PLAY_MODE` `UT_SORT_FIELD` `YT_LANG`
  `UT_START_RESULTS`）就地写回用户那份。**理由**：一个每开一次会话就要重按一遍的偏好
  等于没有偏好 —— 链本身读得好好的（实测过），`t`/`l`/`v`/`o`/`e` 改的只是内存，
  缺的从来只是回写这一步。**不新增第九个命令**：D14 要求每个功能都有 agent 面，
  而"设一个偏好"的 agent 面早就存在且早就有文档 —— 就是这条链里的那个 `KEY=value`
  文件本身；为它造一个 `ut-config` 是 D21 否掉 `ut-search` 的同一笔账。
  三条硬约束与这半条一起定，机制细节在 `AS-BUILT-contract.md` §5：

  - **注释与排版原样保留**，只有值会动 —— 会重排注释列的实现要去依赖那个知道中文一字两格的
    宽度层，而宽度层住在渲染路径里；按字节补齐会把中文注释全部错位，且文件仍然合法、
    什么都不报。
  - **被环境变量压住的键拒绝写，并说一次** —— 环境每次启动都压过文件，写下去就是记一个
    程序下次读到、然后扔掉的值。
  - **写是延后的**（脏位 + 空闲 tick / 退出时冲刷），不是按键即写 —— `t`/`l` 可以连按。

  **进公共 API 的**（D13 的表）：链的顺序、两个文件的位置、键的前缀命名空间、"缺出厂文件 = 2"，
  以及"出厂那份永不被写"。键的清单本身不在这里，在 `AS-BUILT-contract.md` §5。

  **重开条件**：出现第二种宿主形态时，"出厂默认值是一个**文件**"这一半可以重开 —— 譬如 Go 版
  把它编进二进制，那时 `<checkout>/config` 不再存在。**链的顺序与键的命名空间不跟着重开**：
  那是调用方与 agent 已经在依赖的东西。**写回那半条的重开条件是另一个**：出现第二个会写
  用户那份的进程 —— 今天"不加锁、后写者赢、丢的是一个偏好"够用，两个写者就不够了，
  那时才谈锁或一份机器自己拥有的文件。
...
是**正当终态**，不是等待状态。这一节存在只为一件事 —— 万一哪条触发真的成立，界定"值得移植的
是什么、代价怎么算"，免得那一天从头吵一遍。（引擎在任何情况下都不移植，见 D5。）任一为真，
**播放器**移植才值得它的风险：
...
2. **真的要单文件分发**（推翻 D1）。
3. ~~真的要支持 Linux~~ —— **这条已于 2026-08-29 被 patch 掉大半**：`resolve_nc_unix`
   按能力探测 netcat 变体（`nc -U` 或 `ncat`），主流发行版装一个带 `-U` 的 netcat 就通
   （`ARCHITECTURE.md` §26）。剩给 Go 的只有 bash 3.2/5 双版本矩阵与
   「busybox-only 环境连变体都装不了」—— 两者都归入下面的"次要"档，单独不再触发。

次要、单独不足以触发：流式搜索结果；每次按键 fork `stty`；想要带编译期保证的测试套件；
bash 3.2/5 双版本行为矩阵；busybox-only 环境（装不了任何 netcat 变体的地方）。
...
```
  P2  Go TUI          ┐ 可选项（§9）：服务于分发与重构安全，由触发条件把关
  P3  播放器移植      ┘ ——— 两条实质触发（MCP、单文件分发）今天都没成立
```

**两条都是可选项，今天都不动** —— §9 的实质触发一条都没成立，Linux 那条已被变体探测
patch 掉（§9 第 3 条）。不动是正当状态，不是欠账。
