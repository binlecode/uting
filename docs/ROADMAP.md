...
与 `SPEC-system.md` 配套：那份讲这套东西**是什么**，这份记录它**该变成什么**、为什么、
以及每一步开工前必须先成立的条件。
...
> 契约与架构的现状在 `SPEC-system.md`，怎么一步步变成这样在 git log 里。一条决定在这里只以
...
（D9）。**架构、函数图与全部契约见 `SPEC-system.md`** —— 那份与代码同步，这里不复述。
...
- **D15 —— 这批功能在 shell 版做，不等 Go 重写。**
  ...
  2. **这批功能的难点不在语言。** 队列/列表/历史是**状态与数据模型**问题（存哪、什么格式、
     ...
     （`SPEC-system.md` §9.2/§9.3），队列与播放列表是同一套模式的第二次应用。
  ...
  （`SPEC-system.md` §28）对数据结构的表达能力有限，**没有关联数组**，所以持久化只能落到
...
- **D9 之后这份文档要多装一半：引擎契约**（见 D3）—— `<engine>-search` 的结果 envelope、
  ...
  只能照抄 `yt-resolve`。
- **前置**：抽取的来源 `SPEC-system.md` 必须与代码同步 —— 从一份自知过期的文档里抽契约，
  抽出来的仍是过期的。今天成立。
...
- 格式沿用套件已有的那套：**一实体一 JSON 文件 + `mkdir` 锁 + 原子 temp+mv**（`SPEC-system.md`
...
- **归第七个命令**（`ut-queue`）：播放器一行不改，符合"加能力=加文件"的既有形状。代价是**两个
  进程都想管同一个播放器**，而 `players/` 只有一个所有者是 `SPEC-system.md` §9.2 的硬不变量 —— 这条路要先回答
  "谁 reap、谁写 state file"。

**验收（对每一条都成立，否则不算做完）**：人能在 `uting` 里用，agent 能用一个动词加 `-j` 做同一件事，
契约进 `SPEC-system.md`，回归进 `tests/`。
