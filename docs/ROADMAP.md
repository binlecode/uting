...
- `shellcheck --severity=warning`（**2026-08-25 八个脚本重测**）共 **17 条**：SC2128×5、
  SC2178×4、SC2034×3、SC2174×3、SC2054×2。上一版是 14 条／六个脚本（2026-08-23），
  **多出来的三条全部来自后加的两个状态命令**：`ut-playlist` 与 `ut-history` 各一条 SC2174
  （`mkdir -p -m 700` —— 与 `ut-play` 那条同形、同为已知可接受项：mode 只作用于最深一层，
  所以两处都在事后再 `chmod`），外加一个 SC2034。收听历史那一批改动本身**净增 0 条**。
...
- **SC2034 那三条里有两条是真的**（`uting` 的 `PLAYLIST_NAME`、`CURRENT_PLAY_URL`：各写 3–4 处、
  一处都不读，是单向状态而不是噪声；第三条 `C_YELLOW` 是有意留空的，理由写在调色板那一段）。
  它们不随本轮改 —— 删 TUI 里的状态是它自己的一次改动，归 `audit-conformance` 那条线。
