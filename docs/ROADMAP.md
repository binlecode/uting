...
- **D16 —— 登录状态可见，但只可见到"发不发"这一层，不做鉴权探测。**
  ...
  这条界线是量出来的，不是猜的：本机实测 2026-08-26 —— 从 chrome 提取到 3159 个 cookie，
  B 站仍然只给匿名音频档（Hi-Res 与 Dolby 不在 format 表里），因为那个 profile 不是大会员，
  而带 cookie 那次调用**没有出错**，所以 `retried` 也是 `false`。
  两个字段都不是登录裁决（`AS-BUILT-engine.md` §8.2）。
