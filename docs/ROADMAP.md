...
**七个平级可执行文件，一层，无库**：播放器 `ut-play`、人机面 `uting`、两对引擎
（`yt-search`/`yt-resolve`、`bili-search`/`bili-resolve`），加一个持久状态存储 `ut-playlist`。
站点知识只在引擎对里，播放只在播放器里（D9），**用户级持久状态只在存储里**（`ARCHITECTURE.md` §9.4）。**架构、函数图与全部契约见 `ARCHITECTURE.md`** —— 那份与代码同步，这里不复述。
...
- `shellcheck --severity=warning`（**2026-08-25 八个脚本重测**）共 **15 条**：SC2128×5、
  SC2178×4、SC2174×3、SC2054×2、SC2034×1。走到这个数的两步：14 条／六个脚本（2026-08-23）
  → 17 条，**多出来的三条全部来自后加的两个状态命令**（`ut-playlist` 与 `ut-history` 各一条
  SC2174 —— `mkdir -p -m 700`，与 `ut-play` 那条同形、同为已知可接受项：mode 只作用于最深
  一层，所以两处都在事后再 `chmod` —— 外加一个 SC2034；收听历史那一批改动本身**净增 0 条**）
  → 15 条，删掉 `uting` 里那两个单向变量。
  SC2128/SC2178 那 9 条**全在 `uting`、全是假阳性** ——
  `filter_live` / `read_query_input` 的 `local query=""` 与全局 argv 数组同名，shellcheck 不跟踪
  作用域。要做的是改名消噪，不是改逻辑。四个引擎与播放器文件里 SC2128/SC2178 是零。
- **SC2034 只剩一条，且是有意的**：`C_YELLOW` 是留空的调色板槽位，理由写在调色板那一段。
  另外两条是真的单向状态 —— `uting` 的 `PLAYLIST_NAME` 与 `CURRENT_PLAY_URL`，各写 3–4 处、
  一处都不读 —— 已删；`CURRENT_PLAY_URL` 连 `apply_player_record` 里专为它取的那个 jq 字段
  一起去掉，`clear_play_state` 的字段序仍与声明块逐行对齐。

### 6.2 功能完整度：**契约层面为零缺口，功能层面 P4 已补齐**

两把尺子，必须分开量，否则结论一定错：

- **契约完整度：到了。** envelope、退出码、生命周期、引擎契约都实现且有回归覆盖
  （`ARCHITECTURE.md` §14/§27）。
- **功能完整度：到了。** 曾经缺的正是 P4 那三条（播放列表、队列、历史），三条都已落地，
  每条都带着自己的动词和 `-j` envelope。按 §0 的定位这就是清单的全部 —— 拿 kew / ncspot /
  termusic 当基准评这套东西仍然是错的，那是通用本地播放器的清单，不是这里的。
...
| **agent 可驱动的那个**（即 §0 的定位；今天占着 YouTube 与 B 站两格） | **功能够**，契约有一份可以照着写引擎的独立文档（`AS-BUILT-contract.md`） |
...
收听历史（0.3.5）。**as-built 在 `ARCHITECTURE.md` §9.4/§9.5/§9.6 与
`AS-BUILT-contract.md` §1.5/§1.6**；施工文档 `PLAN-listening.md` 已随最后一条删除。下面留下的是
**决定与理由**（哪条为什么在范围内、哪条为什么不做、重开条件），不是待办清单。

**已定并已兑现的一件事：状态住在哪。**

- 一个用户级状态目录（与 `players/` 的**运行时**状态分开 —— 那个在 `$TMPDIR` 里，重启即失，
  正是播放列表**不能**待的地方），落在 `${XDG_STATE_HOME:-~/.local/state}/uting/`，
  `UT_STATE_DIR` 可覆盖。**队列是唯一的例外，而且是有意的**：它归播放器，住在运行时状态里（见下）。
...
|---|---|---|---|
| 播放列表管理 | **已落地** | 它是三条里唯一纯粹的状态题，跑通的状态层后两条复用 | —— |
| 播放队列（**连四个运行时播控动词一起发**，见 §11） | **已落地** | 今天播完一首就没了；`ut-play` 的生命周期只认"一个播放器一次播放" | 归播放器，队列是播放器自己的运行时状态（`ARCHITECTURE.md` §9.5，含实测定死的两条：mpv 必须放后台 `wait` 才收得到 `--next`；退出的原因来自子进程自己的变量，不解码 mpv 的 rc） |
| 收听历史 | **已落地** | 与 `--status` 的 `failed[]` 明确不同：那个在 `$TMPDIR`、有界、只记失败 | 第八个命令 `ut-history`（`ARCHITECTURE.md` §9.6）：默认开、`UT_HISTORY=0` 关、写入点在播放器、JSONL 每行 < 4096 字节。**重开条件**：这套东西要跑在共享账号上时，默认值重新讨论 |
...
1. ~~**播放列表管理**~~ —— **已落地**：用户级状态目录、一实体一 JSON、`mkdir` 锁、`-j` 读 +
   动词写，外加 TUI 的两个键。它把状态层跑通了，后两条复用；它也顺带覆盖了"收藏"（一个具名列表）。
2. **播放队列 + 运行时播控动词** —— **播控四个动词已落地（2026-08-24）**，`uting` 的 Space/`[`/`]`
   改调动词、净删了 TUI 的 IPC 写代码，队列还没开工。架构问题在这一步解决（见下），也是三条里唯一会动到
   `ut-play` 生命周期契约（D3 冻结面）的一条，所以要趁契约还只服务两个引擎的时候动，而不是更晚。
   它是"一个正在消费的播放列表"，所以排在 1 之后。**`--pause`/`--resume`/`--seek`/`--seek-to`
   同批发**（判据作废的理由见 §11）：`--next` 与它们完全同形，冻结面开一次比开两次便宜。
3. ~~**收听历史**~~ —— **已落地**：放最后，因为它建在 2 的播放循环上。写入点随队列的归属定在
   播放器 —— detached child 在**曲终或自己收到信号时**按名字调 `ut-history --record`；只在曲终写，
   历史就只记得下没人打断的那些首。让这句话在 `--stop` 下也成立，代价是一条实测出来的 bash 3.2
   事实：两条信号同一瞬间到达会在两个 handler 之间打死子进程（`ARCHITECTURE.md` §9.5 事实 2）。

**队列归谁：已定并已落地 —— 归播放器，且队列是播放器自己的运行时状态
（`players/<id>.queue.json`），不是一份它不拥有的外部文件（`ARCHITECTURE.md` §9.5）。** 下面两条是被否掉的路，留着是为了
挡住重开：**一条活过重启的队列就是播放列表**，这是判据。

- **归播放器，但读一份外部队列文件**（PLAN §3 的路 A —— 与采纳的路 C 只差队列住在哪）：
  `--status` / `--stop` / `players/` 一个所有者的不变量保持不变，agent 面天然继承（队列状态进
  `--status` envelope）。代价是播放器要读一份**它不拥有的**队列文件，谁写谁读要另立规矩。
...
契约进 **`AS-BUILT-contract.md`**（契约不再住 `ARCHITECTURE.md`），回归进 `tests/`。
...
- **运行时播控动词 `--pause` / `--resume` / `--seek` —— 决定已重开，并入 P4 步骤 2，且已于 2026-08-24 落地。**
  原来的封印是 `ARCHITECTURE.md` §26 的一条判据：**"只有当调用方真的没法直接跟 socket 说话时，
  才加动词"**，而有 shell 的 agent 能自己 `nc -U`，所以条件不成立、这批动词挂在 §9 触发条件 1
  （MCP）上。**这条判据现在作废，理由是它解释不了本仓已经发出去的 surface：**

  1. **`--set-volume` 就是反例。** volume 和 pause 是**同样**能被 socket 直接改的属性，
     判据对两者一字不差地成立 —— 而 volume 做成了动词。所以真正的取舍从来不是那条判据，
     是"TUI 先要什么就有什么"。一条解释不了自己产物的规则不是规则。
  2. **步骤 2 会把它第二次推翻。** 队列要 `--next [--id ID]` —— 对一个在跑的播放器、走同一个
     socket、歧义同样退 4 的 mutation 动词，和 `--pause` **完全同形**。`--next` 一发货，
     再拒绝 `--pause` 就只是任意。
  3. **代价是反的。** D3 说动冻结面是一次"刻意且有记录的行为"。现在一起做是**一次**契约变更；
     分两次做是把冻结面开两回。
  4. **代码已经存在，只是长在错的文件里。** `uting` 的 `toggle_pause` / `seek_relative` 早就跑通
     了 IPC。按"correctness 加在**下面**"，这是把逻辑往下搬，净删 TUI 代码，不是净增播放器代码。

  **原判据里有一半是对的，保留**：`uting` 每秒刷新的**读**路径（`fetch_play_times`，一次连接读
  四个属性）绝不改成动词 —— 每 tick fork 一条进程链是真代价。判据错在把**读**的延迟理由套到了
  **写**上：pause / seek 是一次按键一次，不是每 tick 一次。

  两条设计约束原样有效（§26 早已写下，不必重开）：`--seek` 相对必须带符号、绝对另用 `--seek-to`；
  `--toggle-pause` 不做（`cycle pause` 不回值，envelope 只能猜）。**MCP 仍是 non-goal** —— 这批动词
  和 §9 触发条件 1 从此**脱钩**：它们本来就该在，与要不要 MCP 无关。
  展开见 `ARCHITECTURE.md` §9.3 与 `AS-BUILT-contract.md` §1.1 —— as-built 是随码同步的，
  这批动词落地时 §26 那条非目标随之作废。
...
- **引擎边界** —— 第三方音源的引擎对进本仓，还是各自成仓、只靠 `AS-BUILT-contract.md` 对齐？
  原本"取决于抽不抽得出一份能照着写引擎的文档"——文档在了（§3 的引擎 envelope + §6 的加引擎
  清单），所以这个问题现在只剩**要不要收**，不再是**能不能靠文档对齐**。
