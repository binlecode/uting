...
- **不做收藏**：它是"一个名字固定的播放列表"，而播放列表在范围内（D14）。一个能力两种拼法，
  正是 D10 那条"一名一物"要防的。
- **不做下载器、频道订阅**：不在这一轮。**它们不是 non-goal，是未排期**，各自的未决问题记在 §11。
- 不做 MCP（由 §9 的触发条件决定是否解除）。

**在范围内**：**这套音源上的收听完整度** —— 播放列表、队列、收听历史（D14/D15）。
它们服务于这套音源上的收听，不是给通用本地播放器当替代品；边界与那条硬约束在 D14。
**建成的是什么不在这里**：`ARCHITECTURE.md` §26 的范围说明、`AS-BUILT-player.md`
§9.4/§9.5/§9.6，以及 `AS-BUILT-contract.md` §1.1/§1.5/§1.6 的三张契约面。
...
**八个平级可执行文件，一层，无库** —— 而这句话之后的每一个细节都不属于这份文档。
拓扑与文件布局、函数图、运行时依赖、bash 3.2 地板、全部契约：`ARCHITECTURE.md` §4/§17/§28
与 `AS-BUILT-contract.md` §4。那些与代码同步，这里一个字都不复述 —— 也不放行数表：
一张随每次 commit 过期的表，是这份文档最容易失真的地方。
...
3. **「有 CLI」不构成差异化** → §7 第 3 条：同格已有"守护进程 + CLI 动词 + JSON"的实现，
   差异化只能落到**契约的严格度**。
...
落定的决定：**D1**（支持面大于差异化，不为分发打包）与 **D2**（照样公开）。**仓库已公开**，
所以这笔账不再是"要不要付"而是从此在付 —— 这一节列的就是预期的 issue 形状。
...
引擎契约，有回归覆盖）与**功能完整度**（D14 的那三条收听功能）—— **两把都到了**。
...
6. **agent 驱动两面都成立，第二面支持 shell。** 使用层面语言无关。开发层面：shell 利于**迭代**
   ...
   **三次 `set -e` 回归** —— 正是编译器一次消灭的一类）。不足以压过第 2 条，
   但方向相反，必须记录。
...
- **D2 —— 但照样公开仓库，定位为参考实现，不承诺打包。** README 写明这是"一个可用的 shell 实现 +
  ...
  **已落地（2026-08-27，`github.com/binlecode/uting`）**：README 的 Status 认领这个身份，
  Requirements 直说依赖与平台现实（macOS 优先；Linux 需要支持 `nc -U` 的 netcat），
  What it is not 直说 §0 的 non-goals，且没有 vendor 任何依赖。
  **公开不改 D1**：仍不打包、仍无 installer、仍无 `v*` tag（D13）。
...
- **D6 —— 发布名 `uting`（§3）**；一个命令一个名字，不留第二种拼法。
  挪威语里 `uting` 是真词（"陋习"）—— **当彩蛋接受**：仓库已以这个名字公开，
  备选 `tingyu` / `qingyin` 就此作废（瑕疵本身记在
...
- **D7 —— Go 版只发一个二进制加子命令，绝不发三个通用名的可执行文件。**
  ...
  **永不发布 `bin/yt`**（§5、D10）。上表是草稿：`resolve` 刻意不进子命令表（它只被播放器调），
...
- **D20 —— 第三对引擎：未定。喜马拉雅已实测判死，网易云是量过的候选。**
  选喜马拉雅原是为了证明 D19（一个 source 两条 stack）。**2026-08-28 实测判死，整对不做**：
  它的 resolve 半边其实很健康（免费条目 `curl` 直出、比 yt-dlp 快 3 倍、连打十次零风控），
  但**搜索半边过不去** —— 一份真登录的 cookie jar 在 `www` 端点与匿名**逐字同答**
  `riskLevel:5`，`m` 端点认了登录、却改口要 `webtk`，而 **`webtk` 不是 cookie**：
  它是每请求现算的令牌，住在 jar 之外，所以"浏览器 profile → 导 jar → `curl -b`"那套机制
  在原理上就够不到它。搜索页本身也是空壳（`__INITIAL_STATE__` 里只有 SEO 元数据）。
  唯一的通法是把混淆的反爬 bundle 逆进 bash —— 违反"引擎只做薄薄一层"，且反爬令牌持续腐化。
  引擎 = 两个动词（D9）：只有 resolve 没有 search 的东西不是引擎。
  ...
  **候选是网易云**：搜索端点明文、公开、**零凭据**，形状与 B 站完全一致，零新机制，
  且 `fee` 就在搜索响应里 —— `access:"preview"` 的判据现成（见 §11 第一条）。
  代价如实认领：它证明不了 D19，D19 要另找展示件，或接受它暂时只是一条写下来的原则。
  **重开条件**：喜马拉雅 `m` 侧不再答 `webtk缺失`（判死日期 2026-08-28），
  或有人真的要第三个音源 —— 那时从网易云起步，不从喜马拉雅。

- **D21 —— `ut-search`（统一搜索入口 + fan-out）：不做。**
  它存在的理由**只有** fan-out + 合并信封；没有那个能力，它就是每个 `X-search` 的第二拼法，
  D10 直接毙掉 —— 所以砍 fan-out 就整个命令跟着砍，不留一个纯转发壳。
  **2026-08-28 试建后撤回**：两个引擎的 fan-out 是真能力（跑通了：单引擎路径逐字等于该引擎、
  轮转交错、`-s duration` 全局归并、部分失败信封），但它要的代价是**第九个命令**、
  **冻结面上一种新的信封形状**（`failed` + `engines{}` 的部分失败语义，一旦发货就再不能改）、
  以及 TUI 三处 engine 来源的修正。而第三对引擎已无限期推迟（D20），
  收益的那一半——"加音源不加 agent 要认识的名字"——不会来。两个引擎，人面 `e` 键换源就够。
  **重开条件**：第三对引擎落地。那时先修两处冷读击穿点，它们今天触发不了
  （没有合并信封就没有带**行级** `engine` 的信封）：`ut-playlist` 的 `results` 分支只读
  信封级 `.engine`、缺了直接 error 且无视行级；`uting` 的 `build_all_rows` / `FOCUSED_PAYLOAD`
  同样把信封级 engine 当成行的 engine。
...
1. **真的要 MCP**（解除 §0 的那条 non-goal）。同格竞争者已经做了，并且给出了可抄的工程细节
   —— `spotatui` 的默认关闭 +
...
**落地的条目从这张表里删掉，不留"已完成"那一行**：P1（转公开）与 P4（收听完整度）
如今分别住在 D2 与 D14/D15，实际长成什么样住在 as-built 文档里。
...
- **B 站的一条搜索结果 ≠ 一个可播对象 —— 合集至少要标出来。** 实测搜"周杰伦"，前三条全是
  ...
  **哪一种都行，但"不标"不行。** 注意这是**引擎的**活（形态是站点事实），按"correctness 加在下面"
  ...
  三种一起解，不要一站一个补丁。
  **面已经落地**（2026-08-28）：行带必填的 `kind`/`access`（`AS-BUILT-contract.md` §3），
  **但两个在产引擎都无信号可算**，恒印 `track`/`full`（`AS-BUILT-engine.md` §7.2）——
  引擎说它知道的，不猜它没有的。本条等的是一个带真信号的引擎（网易云的 `fee` 是量过的那个）。
