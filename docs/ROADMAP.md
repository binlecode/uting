...
**建成的是什么不在这里**：`ARCHITECTURE.md` §26 的范围说明，以及 `AS-BUILT-player.md`
§9.4/§9.5/§9.6。
...
引擎契约，有回归覆盖）与**功能完整度**（P4 的范围，已落地）—— **两把都到了**。
...
- **D14 —— 收听完整度属于本项目：队列、播放列表、收听历史在范围内。**
  ...
  **硬约束：每一条都必须有 agent 面** —— 一个只有按键、没有动词和 envelope 的功能，
  在这套东西里是半个功能，而且是把差异化让掉的那半个。这条**不随 P4 结束而失效**，
...
**P4（收听完整度）不在上表**：它建完了，而一个建完的阶段不是优先级。它下面那一节留着，
只为**这一轮没做的三条**和它们的重开条件 —— 建成的东西整个在 as-built。
...
已经满足**（P4，§6），所以今天 §5 那笔账买得到东西了，剩下的只是何时去付。
...
### P4 —— 收听完整度（D14/D15）· 已落地

播放列表、队列、收听历史三条都建完了，**建成的东西以及为什么这么建整个在 as-built** ——
`ARCHITECTURE.md` §26（范围说明）、`AS-BUILT-player.md` §9.4/§9.5/§9.6、
`AS-BUILT-contract.md` §1.1/§1.5/§1.6。这里只剩这一轮**没做**的三条，以及什么会把它们重开：
...
**建完的那三条里唯一还开着的口子**：收听历史默认开（`UT_HISTORY=0` 关）——**重开条件**是
这套东西要跑在共享账号上，那时"这个登录名听过什么"不再是一个人的记录，默认值重新讨论
（`AS-BUILT-player.md` §9.6）。
...
- **前置**：收听完整度（P4）—— **已满足**。否则第一批 issue 会是"为什么没有队列"，而那个问题本仓自己也同意。
