...
**用户级持久状态只在那两个命令里**（`ARCHITECTURE.md` §9.4/§9.6）。**架构、函数图与全部契约见 `ARCHITECTURE.md`** —— 那份与代码同步，这里不复述。
...
收听历史（0.3.5）。**建成的东西以及为什么这么建，全部在 as-built**：`ARCHITECTURE.md`
...
|---|---|---|
| 播放列表管理 | 已落地 | —— |
| 播放队列（连四个运行时播控动词一起发） | 已落地 | 队列归播放器，判据是**一条活过重启的队列就是播放列表**（被否的两条路见 `ARCHITECTURE.md` §9.5） |
