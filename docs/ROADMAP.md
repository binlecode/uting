...
- **D13 —— 版本号走 semver 2.0.0，且版本化的对象是「契约」，不是代码**（2026-08-23 定）。

  `shell/VERSION` 从第一天就长得像 semver，但**没人写下过哪一位在什么时候动** ——
  `CLAUDE.md` 只说"契约或用户可见面变了就 bump"，那句话选不出位数。于是 B-3 删掉三个动词、
  D12 让一批原本能放的 URL 退 1，版本号一动没动。补这一条就是补那个选择规则。
  ...
  **本次落 0.2.0 → 0.3.0**，since 0.2.0（B-1）的破坏性变更：

  | 变更 | 破坏在哪 |
  |---|---|
  | B-3 删 `yt-play`，`--get-url` / `--info` / `--transcript` 移出播放器 | 命令与动词消失（无别名期，见 PLAN §5） |
  | B-2 播放器改走 resolve，`--ytdl-format` 退场 | player record 的 `fmt` 改由 resolve envelope 供给 |
  | D12 `<engine>-resolve` 只认本站 host | 原先能解的非本站 URL 现在退 1 |
  | D10 修订：`yt-tui` → `ut-tui` → `uting` | 命令名本身变了，PATH 入口要重建 |

  加法项（`bili-*` 两个引擎、引擎注册表与 `e` 切源）单独出现的话是 z；与上表同批，
  所以这次只走一次 y。
