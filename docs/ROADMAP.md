...
**在范围内的三条收听功能**（D14/D15，施工计划见 P4）：播放列表管理**已落地**（第七个命令
`ut-playlist`）；播放队列与收听历史还没有。三条都服务于**这套音源上的收听**，且每一条都必须
**同时**对人和 agent 成立 —— 只有按键、没有动词和 envelope 的功能，在这里算半个。
...
**在飞**：`docs/PLAN-listening.md`（P4，三条收听功能）—— 第一条**播放列表管理已落地**
（第七个命令 `ut-playlist` + 用户级状态目录）；第二条落地了它的**播控那一半**
（`--pause`/`--resume`/`--seek`/`--seek-to`，§11 那条判据随之作废），队列与历史未开工。
...
### P4 —— 收听完整度（D14/D15）· **进行中，1/3 已落地**

施工计划与逐条进度在 `docs/PLAN-listening.md`。**"状态住在哪"这个决定已经落地并经过实跑**
（`ut-playlist` + `$UT_STATE_DIR`，见 `ARCHITECTURE.md` §9.4），所以下面这三段是**已成立的前提**，
不是待办；剩下的两条（队列、历史）在它之上建。
...
| 播放队列（步骤 2 里**连四个运行时播控动词一起发**，见 §11） | **刚需** | 今天播完一首就没了；`ut-play` 的生命周期只认"一个播放器一次播放" | 归属**与循环形状**都已定（`PLAN-listening.md` §3，含 pre-mortem 定死的两条：mpv 必须放后台 `wait` 才收得到 `--next`；退出的原因来自子进程自己的变量，不解码 mpv 的 rc）；剩下的是实现 |
| 收听历史 | **刚需** | 与 `--status` 的 `failed[]` 明确不同：那个在 `$TMPDIR`、有界、只记失败 | 默认与写入点已定（`PLAN-listening.md` §4.3：默认开、`UT_HISTORY=0` 关、写入点在播放器）；剩下的是每行的截断与 `--clear --before` 的实现。**重开条件**：这套东西要跑在共享账号上时，默认值重新讨论 |
...
3. **收听历史** —— 放最后，因为它建在 2 的播放循环上：写入点已随队列的归属定在播放器
   （`PLAN-listening.md` §4.3 —— detached child 在**曲终或自己收到信号时**按名字调
   `ut-history --record`；只在曲终写，历史就只记得下没人打断的那些首），
   而那个循环要等 2 落地才存在。

**队列归谁：`PLAN-listening.md` §3 已经定了 —— 归播放器，且队列是播放器自己的运行时状态
（`players/<id>.queue.json`），不是一份它不拥有的外部文件。** 下面两条是被否掉的路，留着是为了
...
- **运行时播控动词 `--pause` / `--resume` / `--seek` —— 决定已重开，并入 P4 步骤 2，且已于 2026-08-24 落地。**
  ...
  **写**上：pause / seek 是一次按键一次，不是每 tick 一次。
  ...
  展开见 `docs/PLAN-listening.md` §4.2；`ARCHITECTURE.md` §26 那条非目标在步骤 2 落地时才改
  —— as-built 是随码同步，不写在建造之前。
