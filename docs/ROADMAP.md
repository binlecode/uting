...
```
  ...
  P3  播放器移植      ┘ ——— 三条今天一条都没成立
  P1  转公开          最低（对现有用户零收益，见下）
```

**P1 为什么垫底**：它是这几项里唯一**对现有用户零收益**的一项 —— 转公开不会让这套东西多播一首歌，
而它立刻要付 §5 那笔环境账（预期九成 issue 是环境问题）。它的前置（收听完整度）已经成立，
所以那笔账今天买得到东西，剩下的只是何时去付。这不改 D2（照样公开、定位为参考实现），
只是把它排在最后 —— D2 决定的是"要不要"，这里排的是"什么时候"。
...
- **前置**：收听完整度（D14）—— 成立。否则第一批 issue 会是"为什么没有队列"，而那个问题本仓自己也同意。
