...
- **D12 —— 一个引擎一套：`<engine>-resolve` 只解自己站的 host。**
  ...
  **要付的账,如实记**：URL-only 音源（Bandcamp / Apple Podcasts / 喜马拉雅）因此**接不进来** ——
  没有引擎对就没有入口（§11）。**重开条件**：若它们成为真实需求,正确做法是**给它写一对**,
  不是放宽 host 校验。
...
- **下一个引擎选谁：候选已经筛过一轮，结论是"没有明显的第三个"。** 记在这里是为了**挡住重复调研**，
  ...
  **Bandcamp / Apple Podcasts / 喜马拉雅**没有入口：D12 之后 `X-resolve` 只认 X 的 host，
  URL-only 不是一条路。它们若成为真实需求，正确做法是**给它写一对引擎**，不是放宽 host 校验。
...
- **收听历史默认开（`UT_HISTORY=0` 关）。** **重开条件**：这套东西要跑在共享账号上 ——
  那时"这个登录名听过什么"不再是一个人的记录，默认值重新讨论（`AS-BUILT-player.md` §9.6）。
