...
（第七个命令 `ut-playlist` + 用户级状态目录），队列与历史未开工。
...
| 播放队列 | **刚需** | 今天播完一首就没了；`ut-play` 的生命周期只认"一个播放器一次播放" | 归属已定（见下）；剩下的是子进程结束时起下一首的实现与 `--status` 的 `queue` 键 |
...
3. **收听历史** —— 放最后，因为它建在 2 的播放循环上：写入点已随队列的归属定在播放器
   （`PLAN-listening.md` §4.3 —— detached child 每首结束时按名字调 `ut-history --record`），
   而那个循环要等 2 落地才存在。
...
- **运行时播控动词 `--pause` / `--resume` / `--seek` —— 决定已重开，并入 P4 步骤 2。**
