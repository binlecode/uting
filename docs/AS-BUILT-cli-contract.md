...
**出厂那份没有任何命令会写。用户那份由 `uting` 写回九个键** —— 见下面「写回」。

**写回（九个键，只写用户那份）。** `uting` 在运行时会改九个设置，它们**就地写回用户那份
...
白名单是硬的：**不在这九个里的键，写回路径根本够不着**（`uting` 的 `pref_value` 既是
...
- **被环境变量压住的键拒绝写，并在屏幕上说一次。** 环境每次启动都压过文件，
  ...
  而且是几个月后才被发现的那种。`uting` 在读配置**之前**记下这九个键里哪些已经在环境里，
