...
**定位、差异化与非目标住在 §1**，驱动决定的六条发现住在 §1.1 —— `ROADMAP.md` 从那里取理由。
...
- **默认值也只声明一次**，在根上的 `config` 里 —— 同一个机制，同一条依赖方向的规矩；
  配置当数据读，绝不 source（§4、D16；键表在 `AS-BUILT-contract.md` §5）。
...
（`RESEARCH-oss-landscape.md` §3）：那层每月都有新项目起量，竞争已经打到终端图形协议封面、
...
（播放列表、队列、收听历史，ROADMAP D14/D15）。其余非目标与已知约束在 §26。

判断一个新功能进不进来，先问这一节；答案是"不"的默认成立，除非定位本身被改写。

### 1.1 分析：驱动决定的六条发现

`ROADMAP.md` §8 的决定从这里取理由。
...
1. **差异化在契约，不在渲染。** 真正难而有价值的是 JSON envelope、退出码分类、脱离终端的
   ...
   那不是护城河，是重写会**删掉**（而非搬迁）的负债。

2. **single-binary 是全有全无。** 一个静态二进制、不要 jq、不要 nc、Linux 能跑、安装一行 ——
   链条里留一个 shell 脚本就整体作废。Go TUI 接 shell 引擎仍是四依赖。所以 "best of both
   worlds" 在**风险**维度成立，在**分发**维度不成立。

3. **agent 对接的是窄动词的 argv + 它们背后的契约。** 没有壳可言 —— 窄动词就是实现本身，
   `ut-play` 与四个引擎都是平级 peer。所以可移植的单位是**播放器**，而引擎可以整条留在 shell：
   它们才是随外部网站变动而频繁改的那一半（第 6 条）。
   **但"守护进程 + CLI 动词 + JSON"这个形状不是本项目独有的**
   （`RESEARCH-oss-landscape.md` §3.3）：`spotify-player` 用不同音源做了同一件事且早于本项目。
...
4. **两个 agent 侧诉求落在播放器/新脸上，且 bash 都给不了。**
   ...
   | 想要的 | bash 为何不行 | 归属 |
   |---|---|---|
   | MCP stdio server | 手写 JSON-RPC 帧、长连接、并发 | 第三张脸（既非播放器也非引擎，ROADMAP D7） |
...
5. **yt-dlp 与 mpv 在任何方案里都是子进程，Go 也一样。** 引擎的真实价值是 **flag 学问**
   ...
   term-osd、socket 路径、reap 规则），以 argv 数组原样搬。移植风险在边缘语义：直播行、
   null 播放量、两种时长拼法、`--stop` 幂等、锁顺序。
...
```
  D0  名字：ut-play、yt-search、yt-resolve、bili-search、bili-resolve、ut-playlist、
      ...
      且**不发**短名（ROADMAP D10）。三条命名规矩，一条对一类受众：
  ...
  D6  不依赖第三方媒体客户端。                                                （§2）
  D7  **没有内核**：八个平级成员，各自把自己的门，各自拥有自己的原语调用。
      它们共享的是**信封**，不是代码 —— 一个共享内核会是每个成员都得知道的东西。（§4）
...
**两个根数据文件的拓扑、以及"数据文件而非共享库"这个决定留在 §4**（D16）。
