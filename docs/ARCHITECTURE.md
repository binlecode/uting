...
- **封面画出来是什么样子，两个套件都证不了**（`AS-BUILT-tui.md`「封面」）。
  `tmux capture-pane` 拿到的是字符网格，图形协议的像素不在里面，所以 `capture-pane` 那条
  "版面靠一帧进文档来证"的路子在这里也不通。**已证的是**：发没发、发对没发对、
  以及画图有没有弄坏这个 TUI（tmux 段整体跑 `UT_IMAGE=on`，它抓到过三个真 bug）。
  **明知没有的是**"退出后终端不残留图像"这一眼 —— 它依赖的 `image_clear` 与退出 trap 都在，
  但那一眼本身要一个人盯着一台真 kitty/Ghostty 看，**如实记为「实测」而非「已证」**。
  补法不是加替身，是在一台真终端上跑一遍。
