...
不亚于为人设计。这份是伞状的 **as-built 文档**：架构、功能结构、支持的工作流、模块契约，
以及它们背后的理由 —— 它是一份**追着代码走**的描述，每次动到架构或契约的落地都会重新同步。
每个事实只住在**一个**小节里，其余地方一律指过去。

范围是整套套件。当这份文档重到某个面值得单开时，就分出一份 `AS-BUILT-<scope>.md`，
届时"一个事实一个小节"的规矩横跨整个文档族继续成立。**已经分出去五份**：
`AS-BUILT-contract.md`（冻结的 CLI 面 —— 下面第三部分是墓碑）、
`AS-BUILT-engine.md`（一切与站点相关的：搜索、解析、登录/PO-token 探测、句柄文法 ——
下面 §7、§8.2 与 §10 是墓碑）、`AS-BUILT-player.md`（播放器、队列与两个持久存储 ——
§8、§9 是墓碑）、`AS-BUILT-tui.md`（人机面 —— §11 是墓碑），
以及 `AS-BUILT-verification.md`（风险登记与验证矩阵 —— §25、§27 是墓碑）。
**一个被搬走的小节保留它的编号**，所以一条旧引用只需换文件名，别的什么都不用改。
这份文档**不是**什么：不是排期（`ROADMAP.md`），也不是在飞的工作（`PLAN-<topic>.md`）。
四个阶段的定义在 `CLAUDE.md`。

**这份文档留下的是图、流程、伪码与决定**；每一样东西具体**怎么落地**，在它自己那份
as-built 里。

- 播放器（与站点无关）：`shell/ut-play` —— 负责播放，并拥有 detached 生命周期
- YouTube 引擎（一对）：`shell/yt-search`（查询 → 结果）、`shell/yt-resolve`
  （句柄 → 流 URL + 请求头，外加 `--info` / `--transcript`）
- Bilibili 引擎（一对）：`shell/bili-search`、`shell/bili-resolve`（`--info`；该站不供字幕，
  所以没有 `--transcript` 那一半）
- 持久状态的两半：`shell/ut-playlist`（人存下来的）、`shell/ut-history`（播放器播过的）
- 交互式界面：`shell/uting`（架在动词之上的自有胶水层；不引入额外依赖）
- 面向调用方的表面：每个动词自己的 `-h`/`--help` · 入门指引：`README.md`
- 运行时依赖：引擎里的 `yt-dlp` + `jq`，以及 `curl` —— 它是 `bili-search` 的**必需项**
  （就是那个引擎的传输层），在 `yt-resolve` 里是可选的（探测用）；播放器里的 `mpv` + `jq`；
  `--set-volume` 用到的 `nc`。没有 fzf、没有 TUI 框架 —— 只有最基础的原语。
- 版本号只声明一次，在 `VERSION` 里；每个入口点自己读它、打印自己的名字（§4）。
...
## 2. 所有权模型 —— 两个面，都 100% 自有

整条路径上没有任何有主见的第三方媒体客户端。所有站点相关的编排都活在我们自己的代码里；
外部原语只干通用的、与站点无关的重活，且每一个都被隔离在**单一接缝**之后（§5）。

```
   人机面            ──►  uting                      （交互式：自绘菜单、detached 播放）
   LLM/agent 面      ──►  ut-play                    （播放 + 生命周期，与站点无关）
                     ──►  <engine>-search            （查询 → 结果）
                     ──►  <engine>-resolve           （句柄 → 流 URL + 请求头、元数据、字幕）
   原语              ──►  yt-dlp · mpv · curl        （基础件，在接缝后可替换）
```

它们每一个都是暴露在 PATH 上的平级成员；底下没有一个隐藏的内核，彼此之间也没有共享库（§4）。

**所有权为什么要紧（D6）。** 调研过的替代品都被否决为运行时依赖：`ytfzf` 已休眠
...
**这条所有权的线今天划在哪（D7）。** 它曾经是"拥有内核"；自从播放器与引擎拆开之后，
它变成了**拥有接缝**。站点知识归自己所有，并被关在一对引擎里；播放与生命周期归自己所有，
...
```
  ...
  D2  拆分时**退役**。原为：光秃秃的 `yt "query"` → 列表输出。搜索如今是它自己的动词
      （`<engine>-search`），而 `ut-play` 拿到一个非句柄时会点名那个动词，而不是去猜。（§6）
  ...
  D4  uting 画**自己的**菜单（不用 picker/TUI 框架）并把活委派出去：
      ...
      过滤 → 纯 bash。                                                        （§11）
  D5  套件里任何地方都不用 fzf / 交互式依赖。                                 （§11）
  D6  不依赖第三方媒体客户端。                                                （§2）
  D7  拆分时**退役并反转**。原为：一个共享内核，动词是薄薄的门、exec 进内核。
      如今没有内核：八个平级成员，各自把自己的门，各自拥有自己的原语调用。
      它们共享的是**信封**，不是代码。                                        （§4）
  D8  uting 只组合那些**动词** —— 绝不碰引擎的内部，也绝不碰 mpv，
      除非经由播放器已经公布出来的那个 socket。                    （AS-BUILT-contract.md §2）
  D9  detached 句柄 = 一个单调的 mktemp token，**不是** pid：socket 路径在启动前就已知，
      且它对 pid 复用免疫；pid 只留着做存活判断。运行时控制（--set-volume）
      搭的是 mpv 每实例的 IPC。                              （AS-BUILT-player.md §9.3）
  D10 TUI 里**不用 emoji**。它画的每一个字形都是文本呈现（"非图形"），
      而标题在显示前会被剥掉 emoji。这不是装饰：正是它让宽度表**精确**而不只是保守，
      因为一个文本呈现的字形不可能背着表带上 U+FE0F 变成两格。
      17 个字形，封闭库存。                                      （AS-BUILT-tui.md §11）
  D11 一个 detached 播放器**没有键盘**。detach_play 把子进程的 stdin 重定向到 /dev/null，
      而 run_mpv 在 detached 路径上加 --input-terminal=no。这是 D9 那个进程组的**后果**，
      不是一个独立的选择：`set -m` 压制了 bash **自己**给后台作业的自动 /dev/null，
      于是子进程继承了调用方的 tty，mpv 与 TUI 逐键争抢。       （AS-BUILT-player.md §9.1）
  D12 引擎**名**就是命令前缀。`--engine yt` 靠字符串拼接找到 `yt-resolve`，
      于是加一个源不会在播放器或 TUI 的任何地方加出注册表、映射表或 case 分支。（§4）
  D13 一个引擎靠**有没有那个动词**来声明能力。`bili-resolve` 没有 --transcript，
      因为该站不供字幕 —— 而不是给一个永远答"没有"的动词，
      那种东西调用方分不清它与"今天不走运"。                （AS-BUILT-contract.md §1）
  D14 **持久**状态是一个自己的命令，且住在 $TMPDIR 之外。`ut-playlist` 拥有用户级存储；
      播放器的 `players/` 保持只属于运行时，随重启一起死。存下来的记录是
      {engine, url, …} —— 也就是 `ut-play` 的那两个参数，所以一条记录是一次**调用**，
...
**引擎名就是命令前缀（D12）。** `--engine yt` 靠字符串拼接找到 `yt-resolve`
...
`~/bin/ut-play` 是一条指进 checkout 的符号链接（ROADMAP D1/D2），所以一个朴素的 `dirname`
...
**mpv 不再运行 yt-dlp**（步骤 B-2）。`run_mpv` 传的是 `--no-ytdl` 加一个引擎已经解出来的直链。
在此之前，任何一次播放里**最后**那次抽取都不是我们发起的：我们分类不了它，
只能通过 `--ytdl-format` / `--ytdl-raw-options` 去间接影响。现在是：
...
这也是为什么拆分是按*操作*而不是按站点（ROADMAP D11）。
...
**由构造保证的非交互（D1/D3）。** 除 `uting` 之外没有任何动词会提问。"没有句柄"那道守卫跑在
...
这从前是包装层依赖的一道守卫；如今它归每个动词自己。

**一次调用一个动作。** `set_action` 记下是哪个 flag 认领了这次调用，并拒绝第二个不同的
（`--status --stop` → "conflicting actions"），而旧的"最后一个 flag 赢"的解析会静默丢掉第一个。
...
以及真正被播放的那次抽取发生在哪里"*。自步骤 B-2 起，后半个问题的答案很短：
...
AS-BUILT-contract.md §3）。`bili-resolve` 根本没有 `--transcript` 那一半（D13）。
...
```
   ...
   │  │    host 白名单：不是本站的 host → 退 1（ROADMAP D12）
...
```
   ...
   进程 1 ：ut-play，那个**会返回的**父进程
        detach_play: ensure_state_dir · new_player_id · lock_player_state
             ...
             │      于是播放器活过这个父进程的退出（player §9.1）；stdin → /dev/null（D11）
...
如今住在 `docs/AS-BUILT-contract.md`，也就是 ROADMAP D3/D13 的那个冻结面。
...
产出这套架构的那次重构遵循了一个分阶段、可回退的顺序 —— 一个可复用的模板，
供将来的结构性变更使用：

```
   A  用**当前**的工具把新路径建起来；在 tmux pty 里验证它。
      → 交互路径从不缺席。
   B  重指调用方 / 包装 / 符号链接；跑无头回归。
   C  删掉旧路径 —— 破坏性那一步，放在**最后**且最小；
      删每一个符号之前先用 grep 把闸；再回归一次。
   D  更新文档（本文件、README、usage()）。
   E  最后一次有头（tmux）+ 无头扫尾。
```

原则：**把唯一那一步破坏性动作放在最后、并做到最小；先证明它的替代品；用 grep 给删除把闸，
于是没有悬空引用能活下来。**

**用过两次，而第二次就是它有效的证据。** 播放器/引擎拆分（ROADMAP D9）按 A → E 走了六步，
破坏性那一步靠后且单独：重命名内核（A）、无破坏地把搜索抬出去（B-1）、
把抽取抬出去并在唯一那个耦合步骤里退役 `ytdl_hook`（B-2）、**删掉 `yt-play` 包装**（B-3）、
加第二个引擎来证明那条接缝（C），然后是 TUI 的注册表与改名（D）。
两条性质让它可回退：除 B-3 之外每一步都让旧路径继续可用，
而第二个引擎是在接缝**已经存在之后**才加的 —— 于是"一个新源不改变别的任何东西"
是一次**检验**，而不是一个**指望**。

## 25. 风险登记（设计层面的缓解）
已移出 → `AS-BUILT-verification.md` §25 —— 风险表，以及 §25.1 的缺陷登记册
（已空，留着是为了里面那些更正与 harness 教训）。
...
- **范围说明（ROADMAP D14/D15）：三个收听功能全部已落地**，在 shell 版里，
...
- **队列的编辑 —— 重排、出队、循环、随机 —— 刻意不进 v1。** 那些是对一条队列的操作；
  ...
  （每个都带自己的 `-j` 信封，D14），不是加一个新命令 —— 队列归播放器（player §9.5）。
...
- **对一个 detached 播放器的运行时控制是一组动词**（`--set-volume N`、`--pause`、
  ...
  四个播控动词是在 `--set-volume` 之后到达的，而**那条曾经把它们挡在门外的判据 ——
  "只有当调用方真的没法直接跟 socket 说话时，才加一个动词" —— 已经作废。**
  有四件事把它作废掉，写在这里是因为一条听起来这么有道理的规矩会被反复重提：
  ...
  属于这里的是**代码到底是什么**：
    ...
    - **什么**没有**搬，以及决定它的那个数字。** `uting` 每拍一次的**读**
      ...
      而老判据的**那一半**一直是对的。按住不放的 `9`/`0` 音量键也一样 ——
