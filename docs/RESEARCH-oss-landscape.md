...
- **功能完整度：到了。** `ROADMAP.md` D14/D15 那三条（播放列表、队列、历史）都已落地，
  每条都带着自己的动词和 `-j` envelope。按 `ARCHITECTURE.md` §1 的定位这就是清单的全部 ——
...
| **agent 可驱动的那个**（即 `ARCHITECTURE.md` §1 的定位；今天占着 YouTube 与 B 站两格） | **功能够**，契约有一份可以照着写引擎的独立文档（`AS-BUILT-contract.md`） |
