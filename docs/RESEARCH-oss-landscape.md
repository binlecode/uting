...
> **D1/D2**（不打包但照样公开，仓库已公开）· **§9**（把播放器推向 Go 的触发条件）。
...
它没能推翻发布名 —— 仓库已以 `uting` 公开，当彩蛋接受（`ROADMAP.md` D6）；
备选 `tingyu` 听雨 / `qingyin` 清音两个也都六项全空，记在这里只为存档筛查结果。
...
- **`shellcheck --severity=warning` 基线是一个被跟踪的计数，不是一张干净的体检单**
  ...
  - 走到 22 这个数的几步（**测量史，不是变更史** —— 记它是为了说明这条基线是被跟踪的）：
    15 条（2026-08-25）→ 23 条，配置文件那一批（`b11de01`）给八个入口点各加了一条
...
| 以什么身份公开 | 现在够吗 |
|---|---|
| **参考实现 + 设计文档** | **够** —— 仓库已按这个身份公开（`ROADMAP.md` D2） |
