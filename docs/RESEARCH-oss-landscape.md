...
- **`shellcheck --severity=warning` 基线是一个被跟踪的计数，不是一张干净的体检单**
  （2026-08-25 八个脚本重测）共 **15 条**：SC2128×5、SC2178×4、SC2174×3、SC2054×2、SC2034×1。
  ...
  - 走到 15 这个数的两步（**测量史，不是变更史** —— 记它是为了说明这条基线是被跟踪的）：
...
- **功能完整度：到了。** 曾经缺的正是 `ROADMAP.md` P4 那三条（播放列表、队列、历史），三条都已落地，
