...
5. ~~**手写发射器产出的 PNG，真终端到底收不收**~~ —— **已答，2026-09-03**，在
   Ghostty 1.3.1 上实测（当时那句"本机终端不支持 TGP"已经过时）。收：`a=q` 探测回
   `OK`，`q=2` 不回一个字节，`r=7` 缺省 `c` 时终端按原图比例自适应列宽（192×108 一图
   分到 7 行 × 28 列，与按格子尺寸算出的分毫不差），而格子尺寸要问 `CSI 16 t` 而不是
   假设（实测 16×36 px，不是"宽 1 高 2"推出来的 8×16）。落地后的 why 在
   `AS-BUILT-tui.md`「封面」，本条到此为止。**仍未量到的只剩"退出后残不残留"那一眼**，
   如实记在 `ARCHITECTURE.md`「已接受的残余风险」。
