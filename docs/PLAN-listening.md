...
2. ~~`VERSION` 的 z,单独一个 commit~~ —— `0.3.3`。
3. ~~**§7 的曲间空隙还没量**~~ —— **已量并填回 §7(2026-08-25)**:median 4.3s、p90 5.7s
   (n=12,5 条目 yt+bili 混合队列 ×3,每首自然播完)。**p90 < 8s,预取不进 v1**,触发条件原样
   留在 §7 等下一次复测。
...
- **semver**（D13）：三条都是**加法** —— 新命令、新动词、envelope 新增键。既有调用方一行不用改。
  → 每条落地各 bump 一次 **z**，`VERSION` 单独一个 commit，不随功能 commit 走。
...
- **曲间空隙**：**已量（2026-08-25）—— median 4.3s、p90 5.7s、极差 3.2–5.8s（n=12）**。
  低于 8s 线，**所以预取不进 v1**：§5 步骤 2 第 5 条预先写死的触发条件按原文兑现，不重新解释。
  预算写的是"约 3 秒"，实测是它的 1.4 倍 —— 这正是那条"一个没量过的约 3 秒就是下一次计时事故"
  的意思。
  - **口径**：从第 N 首最后一次报出 `position` 到第 N+1 首第一次报出 `position` —— 声音停到声音
    起，全程读真播放器的 `--status`，不读任何内部计时。5 条目 yt+bili 交替队列跑 3 遍，每首
    `--seek-to duration-3` 让它**自然播完**（不是 `--next`：跳曲不是这里要量的那条路）。
  - **空隙的 92%（中位）就是引擎那一次往返**（resolve 中位 3.8s，由播放器记录改名到新 handle
    的时刻切出来）。剩下的才是子进程发现 mpv 退出、在自己的锁里推进、以及新 mpv 起解码。
    所以这条数字要降只有一个降法，就是预取 —— 循环里没有别的肉可以割。
  - **方向差是真的**：**→bili 中位 3.4s，→yt 中位 5.3s**。慢的是 YouTube 那一侧：`yt-resolve`
    要读浏览器 cookie，还可能再花一次 PO-token 探针（curl）甚至一次匿名重试；`bili-resolve` 一次
    yt-dlp 就够。一条混合队列因此是**必须**的量法 —— 只量一个引擎会得到一个偏乐观的数。
  - 优化路径不变、也不落地：在第 N 首播放期间预解析第 N+1 首 —— 那要引入第二个后台任务和一份
    会过期的缓存（stream URL 数小时就失效），**只在 p90 > 8s 时才付**。这条数字挂在真实站点上，
    上游变了要复测，触发条件不变。
  - 测量件是 `tmp/` 下的一次性脚本，按 CLAUDE.md 既不进 `tests/`（它对着真实站点计时），也不在
    文档里留路径 —— 记的是**口径**，不是文件名。
