...
> 由它落定的决定住在 `ROADMAP.md` —— **D4**（名字）· **D1 的 non-goal**（不做通用
> TUI 播放器）· **D2**（不打包）· **D10**（不做 Go 版）。
>
...
住在 `ROADMAP.md` D4，这里不复述。**下表留档只为一个目的：挡住重复调研。**

| 候选 | 六项结果 / 否掉的理由 |
|---|---|
| `uting` | **全空** —— 采用（`ROADMAP.md` D4）。已知瑕疵：挪威语里是真词，意为"陋习"，当彩蛋接受 |
| `tingyu` / `qingyin` | **全空** —— 备选，未采用 |
| `ut-play` / `ut-search` 等长前缀 | **全空** —— 采用（`ROADMAP.md` D4） |
...
- **五个运行时依赖**（yt-dlp、jq、mpv、nc、curl），全部由用户负责安装并保持可用；且**每加一个
  音源就多一份站点维护面** —— 引擎对是本仓唯一会因外部网站变动而坏掉的地方（`ROADMAP.md` D5.1
  把它关进两个文件，但没让它消失）。
- **通用短名不可用**：由 `ROADMAP.md` D4 兑现（命令名全做过六项筛查，§2），
...
- **功能完整度：到了。** `ROADMAP.md` D6 那三条（播放列表、队列、历史）都已落地，
