...
住在 `ARCHITECTURE.md` §3.1，这里不复述。**下表留档只为一个目的：挡住重复调研。**

| 候选 | 六项结果 / 否掉的理由 |
|---|---|
| `uting` | **全空** —— 采用（`ARCHITECTURE.md` §3.1）。已知瑕疵：挪威语里是真词，意为"陋习"，当彩蛋接受 |
| `tingyu` / `qingyin` | **全空** —— 备选，未采用 |
| `ut-play` / `ut-search` 等长前缀 | **全空** —— 采用（`ARCHITECTURE.md` §3.1） |
...
- **五个运行时依赖**（yt-dlp、jq、mpv、nc、curl），全部由用户负责安装并保持可用；且**每加一个
  音源就多一份站点维护面** —— 引擎对是本仓唯一会因外部网站变动而坏掉的地方（`ARCHITECTURE.md` §3.1
  把它关进两个文件，但没让它消失）。
- **通用短名不可用**：由 `ARCHITECTURE.md` §3.1 兑现（命令名全做过六项筛查，§2），
...
- **`shellcheck --severity=warning` 基线是一个被跟踪的计数，不是一张干净的体检单**
  ...
  **SC2128/SC2178 归零**。这不是把它们修好了：`filter_live` / `read_query_input` /
  ...
  - SC2128/SC2178 那 9 条（塌缩之前）**全在 `uting`、全是假阳性** —— `filter_live` /
    ...
    **四个引擎与播放器文件里 SC2128/SC2178 一直是零。**
...
- **功能完整度：到了。** `ARCHITECTURE.md` §3.5 那三条（播放列表、队列、历史）都已落地，
