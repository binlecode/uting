...
> 由它落定的决定：`ARCHITECTURE.md` 的 **D0**（名字）与 **§1 的 non-goal**（不做通用
> TUI 播放器），`ROADMAP.md` 的 **D2**（不打包）与 **D10**（不做 Go 版）。
>
...
住在 `ARCHITECTURE.md` D0，这里不复述。**下表留档只为一个目的：挡住重复调研。**

| 候选 | 六项结果 / 否掉的理由 |
|---|---|
| `uting` | **全空** —— 采用（`ARCHITECTURE.md` D0）。已知瑕疵：挪威语里是真词，意为"陋习"，当彩蛋接受 |
| `tingyu` / `qingyin` | **全空** —— 备选，未采用 |
| `ut-play` / `ut-search` 等长前缀 | **全空** —— 采用（`ARCHITECTURE.md` D0） |
...
顺带记一笔（真做 MCP 时要用，即 `ROADMAP.md` D10 的重开条件）：字幕这一格在 MCP 生态里
...
- **五个运行时依赖**（yt-dlp、jq、mpv、nc、curl），全部由用户负责安装并保持可用；且**每加一个
  音源就多一份站点维护面** —— 引擎对是本仓唯一会因外部网站变动而坏掉的地方（`ARCHITECTURE.md` D18
  把它关进两个文件，但没让它消失）。
- **通用短名不可用**：由 `ARCHITECTURE.md` D0 兑现（命令名全做过六项筛查，§2），
...
- **功能完整度：到了。** `ARCHITECTURE.md` D22 那三条（播放列表、队列、历史）都已落地，
  每条都带着自己的动词和 `-j` envelope。**"工作时放点音乐"这件事，清单到此为止** ——
  拿 kew / ncspot / termusic 当基准来评是错的，那是通用本地播放器的清单，不是这里要的。

### 5.3 发布硬件

只剩一个真缺项：**Linux 实际不可用**（§4；后由 netcat 变体探测关闭，`ARCHITECTURE.md` §26）。
其余都是可选项。
...
| **陌生人 brew / curl 装** | **不够**，卡点不在功能，在 §4 的环境账 —— 也不是现在想要的（`ROADMAP.md` D2） |
