...
住在 `ARCHITECTURE.md`「八个平级动词」，这里不复述。**下表留档只为一个目的：挡住重复调研。**

| 候选 | 六项结果 / 否掉的理由 |
|---|---|
| `uting` | **全空** —— 采用（`ARCHITECTURE.md`「八个平级动词」）。已知瑕疵：挪威语里是真词，意为"陋习"，当彩蛋接受 |
| `tingyu` / `qingyin` | **全空** —— 备选，未采用 |
| `ut-play` / `ut-search` 等长前缀 | **全空** —— 采用（`ARCHITECTURE.md`「八个平级动词」） |
...
- **五个运行时依赖**（yt-dlp、jq、mpv、nc、curl），全部由用户负责安装并保持可用；且**每加一个
  音源就多一份站点维护面** —— 引擎对是本仓唯一会因外部网站变动而坏掉的地方（`ARCHITECTURE.md`「八个平级动词」
  把它关进两个文件，但没让它消失）。
- **通用短名不可用**：由 `ARCHITECTURE.md`「八个平级动词」 兑现（命令名全做过六项筛查，§2），
...
- **契约完整度：到了。** envelope、退出码、生命周期、引擎契约都实现且有回归覆盖
  （`AS-BUILT-contract.md`「数据契约」、`AS-BUILT-verification.md`「验证矩阵」）。
