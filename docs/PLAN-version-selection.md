...
| P6 | 章节 → 条目视图里的 seek 目标 | **已落**（2026-08-30；§9 的开放问题已拍板，六条记在 §4 的 P6 里。`shell/uting` 一处：章节表随 `--info` 落库、`card_chapter_block`、卡内 ↑/↓ 让给游标、`Enter` 走 `--seek-to`。**零契约动作**——agent 面本来就全。`contract.sh` 268 / `playback.sh` 47 全绿，实测记在 §13。**内容已进 as-built**：`AS-BUILT-tui.md` §11（章节块、三种字重、两块的预算与次序、↑/↓ 的改绑）、`AS-BUILT-contract.md` §1.4、`AS-BUILT-verification.md` §27（含「为什么一条新检查都没加」与重测的 shellcheck 基线）、`ARCHITECTURE.md` §17 的函数表、`README.md`） |
...
#### P6 已拍板（2026-08-30，用户；§9 那条留到 P5 之后的开放问题就是它）

先把 §9 等的那个量测出来：**80x24、按过 `i` 的在播卡今天只用掉 10 行，余 14 行**
（表头+空行 2、脊柱 3、meta 1、info 1、空行 1、进度条 1、提示 1）；前方队列块满员（表头+5）时仍余 ~8 行。
**行不是稀缺资源，所以章节可以是一个开窗的块，而不是一行摘要。**

1. **选章 = 卡内 `↑/↓` 游标，`Enter` 跳。** 数字键被否：`9`/`0` 已经是音量，数字也只够到第 9 章，
   而实测的章节表可以有几十条。落地的同时**音量退回 `9`/`0` 独占** —— 这正是 §12 移交给本条的那一项，
   卡内 `↑/↓` 从此不再调音量。
2. **可跳的地方是「有播放头的那张卡」，不是「主语在播的那张卡」。** 条目主语上章节只是一份目录：
   那张卡没有播放头、没有进度条、没有已播时间，一个没有东西可 seek 的 seek 目标就是目录；
   而 `Enter` 在条目卡上已经是「播放」（P5），让同一个键在同一张卡上说两件事是比少一个入口更贵的代价。
   按了 `i` 的那一条正好在播时，**一次 `Tab` 就到有播放头的那张卡**，章节在那儿可跳。
3. **`Enter` = `ut-play --seek-to <章节起点>`**，在播卡上它今天是空键，所以这是新增而不是改写。
   不重新 resolve —— §3 的表已经把这条轴与另外两条分开了。
4. **块的次序：前方队列块在上，章节块拿剩下的行。** 队列块是这张卡上唯一「不按键也会变」的读数，
   卡片就是为看它存在的；章节是静态目录，按剩余 budget 在游标周围开窗。两块同时在时 80x24 仍余 ~8 行，
   所以这不是二选一，只是谁先拿。
5. **标出播放头当前所在的那一章**：其余章节 dim，当前章不 dim。**不新增字形** —— 封闭字形表不动，
   `YT_ASCII` 与 mono 主题下同样读得出（mono 里 `C_BOLD` 仍在，只有 `C_CYAN` 是空的）。
   游标用列表同款的 2 格 `> ` 标记（`C_MARK`），与「当前章」是两件事、两个信号。
6. **时间列在取数时算一次，不在每帧算。** `fmt_sec` 是子壳调用，卡片 1 秒一跳，50 章就是每帧 50 次 fork；
   起点秒数与格式化好的时间在 `--info` 落库时一起存进平行数组（bash 3.2 无关联数组）。
   每帧只剩一次纯算术的线性扫描来定位当前章。
...
**~~仍然开着的一个~~ 已答（2026-08-30）**：章节在条目视图里怎么选 —— 方向键、还是数字键？
量过了（80x24 的在播卡用 10 行、余 14），答案是**方向键**，六条落在 §4 的 P6 小节里。
...
### tmux 面的 chrome 语言被钉住了（2026-08-29，P5 的 B 步）

本节此前每一条 tmux 断言都是**语言中立**的，而那是被迫的：`YT_LANG` 未设置时的默认是
「zh\* locale 下 zh，否则英文」，也就是面里说哪种语言取决于机器，任何点名 chrome 字串的检查
...
### 章节块与卡内 `↑/↓` 的实测（2026-08-30，P6）

**这一条套件里一个新检查都没加，理由在下面的反例里。** 机制那一半早就有人管：
`--seek-to` 在 `playback.sh` 对着真 mpv 判别过（先前进再回零，顺序是承重的），
`--info` 的 `chapters` 由 `contract.sh` 的**两引擎键集对比**盯着 —— 一边改名就红。
剩下的是画面与键，套件按规矩不断言画面。所以驱动实测，记在这里：

| 看的是 | 看到的 |
|---|---|
| 80x24，`Enter`→`Tab`→`i`（贝多芬第五，4 章） | meta/info 行下多出 `1/4 · chapters` 与四行 `MM:SS  标题`；提示块末尾多两格 `↑/↓ chapters`、`↵ go` |
| 两下 `↓` 后按 `Enter` | 读数从 `00:04 / 35:15 (0%)` 跳到 `18:23 / 35:15 (52%)`，进度条跟着走 —— 与第 3 章起点 `18:22` 相符 |
| 游标在第 4 章、播放头在第 3 章（`capture-pane -e`） | 三种字重都在：1/2 章 `^[[2m` dim、第 3 章 `^[[1m` bold（播放头）、第 4 章 `^[[1;36m> ` 标记 + 正常字重（游标）—— 两个信号互不遮盖 |
| 71 章的长课（`freeCodeCamp Python`） | 窗口封 6 行，表头读 `9/71`，`↓` 到第 7 章时窗口开始滚；时间列因为跨小时统一右对齐到 8 格，不随滚动跳列 |
| 条目卡（列表里按 `i`，未播） | 章节是**目录**：表头 `4 · chapters`，无游标、无 `↵ go`；描述块随之从 6 行收到 5 行，全帧 22/24 行 |
| 62x14 的条目卡 | 章节块**自己退场**（`budget/2 < 2`），帧与 P6 之前一字不差 —— 13/14 行 |
| 80x15 的在播卡（队列 4 条 + 71 章） | 队列块先拿（表头+2），章节块退场，**提示块里的 `↑/↓ chapters`/`↵ go` 一并消失** —— 这一条是本次唯一改过一遍才对的：提示格先按「可能画得出」量、再按「真画出来了」印 |
| 80x24 同一状态 | 两块同在：队列 1/4（表头+3）+ 章节 1/71（表头+6），22/24 行，余量就是那条 `-1` 边距 |

**没有单独驱动的一条**：`i` 之后换轨道时章节块随 info 行一起消失 —— 它们共用同一个 url 门
（`ITEM_URL == CURRENT_PLAY_URL`），与 P5 那条「换轨道时 info 行消失」是同一笔成本。
...
- **F04（标题清洗统一）不进套件，实测记在这里。** 它的可观测面要求**同一个进程里同时有真播放器和 TUI** ——
  ...
  改前同一帧会带着 📚。
- **章节块不进套件，理由与 F04 同族但不同因。** 这里的阻碍不是造不出那一帧（`drive.sh` 造得出），
  而是**断言什么**：块的存在与否是画面（capture-pane 的地界），键的效果是 `--seek-to`
  （`playback.sh` 已判别），字段的形状是 `--info`（`contract.sh` 的键集对比已判别）。
  剩下的差集只有「章节表非空」，而那是**站方内容**：拿哪条视频有几章去断言，红起来是 uploader
  改了描述，不是代码坏了 —— 正是规矩里说的「时着网络的路径」。所以照实说：**这条覆盖套件没有**。
- **不为 P6 加契约动作。** 它的 agent 面**本来就全**：`<engine>-resolve --info -j` 给 `.chapters[].start_time`
  （`ARCHITECTURE.md` §20 已有那行示例），`ut-play --seek-to N` 消费它。P6 给的是键位，
  不是能力 —— 所以它没有违反「TUI-only feature is half a feature」（`ARCHITECTURE.md` §3.5），
  也不需要新动词、新字段或版本位的移动。
