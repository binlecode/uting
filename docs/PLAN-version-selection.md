...
| P6 | 章节 → 条目视图里的 seek 目标 | **已落**（2026-08-30；§9 的开放问题已拍板，六条记在 §4 的 P6 里。`shell/uting` 一处：章节表随 `--info` 落库、`card_chapter_block`、卡内 ↑/↓ 让给游标、`Enter` 走 `--seek-to`。**零契约动作**——agent 面本来就全。`contract.sh` 268 / `playback.sh` 47 全绿，实测记在 §13） |
