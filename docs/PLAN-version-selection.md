...
| P13 | resolve 信封加 `selected` / `selected_resolution`（**§5.2**，本计划两项契约动作里的第二项） | 未开工。**这一条从前没有行** —— 它写在 §5.2 里、叙事上挂在 P5 名下（卡片 meta 行由 `audio` 变成 `234 audio m4a`），于是 §0 从没数过它。2026-08-29 清点时实测确认未落：`shell/*-resolve` 全树无 `selected` 字段。§13 的检查早就写好了（两个引擎的 `selected` 非空且 ≠ `.format`；`playback.sh` 里 player record 的 `selected`）。零额外网络 —— 那两个值已经在 yt-dlp 的原始记录里，今天被扔掉 |
