...
（`fetch_view_once`，与 `--info` 共用那一次 view 抓取）。`parts[]` 的元素**就是条目记录**，所以
`--parts -j | jq '{items:.parts}'` 原样管进 `ut-playlist --add` 与 `ut-play --queue`。
...
悄指向了第 1 P。`--parts` 不受影响 —— 它的 base 是从 `.data.bvid` 拼出来的，从不经过 `TARGET_URL`。
