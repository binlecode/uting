...
| R9 | **Naming drift** | Env knobs missing the `YT_` prefix; a unit-less numeric where the codebase suffixes (`_s`, `_ms`, `_pct`); the short PATH names (`yts`/`ytp`/`ytt`) used in help text or errors where the canonical long names belong, or vice versa in a PATH context; a new envelope field whose name doesn't match its siblings' style. | grep the env-read sites against §16's documented list; scan user-visible strings for the wrong name form. |
...
```
  primitives     yt-dlp · mpv · jq · nc          (external; called ONLY from the core)
        ↑
  core           shell/yt                        (all search/play/resolve/lifecycle logic)
        ↑
...
5. **Run the contract, don't read it** (R8). These are seconds each and settle the class:
   ...
   shell/yt >/dev/null; echo "exit=$?"              # 1 (empty query, D3)
...
|------|-----------|------|-----|
| R3 | shell/yt-tui:NNN | … | move … down into shell/yt |
