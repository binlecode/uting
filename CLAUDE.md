...
- `docs/ROADMAP.md` — **only two things: decisions that must survive a rewrite (each with its reason and its reopen condition), and work not done yet.** No changelog, no landed work, no survey data, and no positioning. `§9` gates the move to Go — a Linux fix that adds a dependency, and MCP, are both refused there. **Landed and therefore gone from it (D14/D15):** playlist management, the queue, listening history, all in the SHELL version; what they ARE lives in `docs/AS-BUILT-player.md` §9.4/§9.5/§9.6. **Two decisions are recorded NOs worth knowing before proposing them again:** a third engine pair (D20 — Ximalaya's search half is unreachable, netease is the measured candidate) and `ut-search` (D21 — a ninth command and a new envelope shape buy too little at two engines). Every feature must ship an **agent surface** (a verb plus a `-j` envelope) alongside its keybinding — a TUI-only feature is half a feature here.
- `docs/RESEARCH-tui-player.md` — the one survey the roadmap's decisions rest on, and it is **two halves with different credibility, which §1 states before anything else**. **§2–§5 are measured** (2026-08-21/25/27): the six-way name screening compressed to one table (ROADMAP D6/D10), the terminal-player landscape by GitHub API (the non-goal in `ARCHITECTURE.md` §1), what publishing the shell version costs, and how ready it is (ROADMAP D1/D2) — including the shellcheck baseline in §5.1. **§6–§11 are read, not run** (2026-08-29): how other players are actually built along five orthogonal axes (where decoding happens · how long the playing process lives · where site knowledge lives · what can drive it · one extraction or many), the four routes to a playable URL on the domestic side, and the agent surfaces that appeared in 2026. Every figure is dated; **re-run the method before reopening anything it settled, and for the read half treat every claim as a pointer to its source** — §12 lists what it could not establish. (It absorbed `RESEARCH-oss-landscape.md`, whose §2–§5 moved here **keeping their numbers**.)
