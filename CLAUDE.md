...
- `docs/ROADMAP.md` — **only two things: decisions that must survive a rewrite (each with its reason and its reopen condition), and work not done yet.** No changelog, no landed work, no survey data, and no positioning. `§9` gates the move to Go — a Linux fix that adds a dependency, and MCP, are both refused there. **Landed and therefore gone from it (D6):** playlist management, the queue, listening history, all in the SHELL version; what they ARE lives in `docs/AS-BUILT-player.md` §9.4/§9.5/§9.6. **Two decisions are recorded NOs worth knowing before proposing them again:** a third engine pair (D9.1 — Ximalaya's search half is unreachable, netease is the measured candidate) and `ut-search` (D9.2 — a ninth command and a new envelope shape buy too little at two engines). Every feature must ship an **agent surface** (a verb plus a `-j` envelope) alongside its keybinding — a TUI-only feature is half a feature here.
