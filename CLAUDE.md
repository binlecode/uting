...
**Status: reference implementation.** Not packaged — no installer, no Homebrew formula, and none planned for the shell version (`docs/ROADMAP.md` D2). Users symlink `uting` (the human surface) and `ut-play`/`yt-search`/`yt-resolve`/`bili-search`/`bili-resolve`/`ut-playlist`/`ut-history` (the agent surface) onto their own PATH. **No short name ships** (`docs/ROADMAP.md` D4) — the human face carries the project's own name, so `~/bin/uting` is a plain symlink with the same word at both ends. A user wanting a short form writes their own alias.
...
tests/contract.sh                                             # …and the live half too: ~95s, 263 checks
...
|---|---|---|---|
| research | `RESEARCH-<topic>.md` | surveying the world **outside** this repo — what comparable projects do, measured data, comparisons. Says nothing about what we will build | **distil the DECISION into a `ROADMAP.md` entry**, filtered by relevance and business need; the survey itself stays here as the evidence, dated, and is re-run or deleted when its measurements go stale — the roadmap holds decisions, never data |
| roadmap | `ROADMAP.md` | the decided: each decision with its reason and the condition that would reopen it, plus what is not done yet. **Positioning and non-goals are NOT here** — they are as-built (`ARCHITECTURE.md` §1), and so is the analysis behind the decisions (§1.1). Its stable citation key is `D#`, which only ever grows; section numbers are renumbered as content moves, so cite `ROADMAP D#` and never a section number | nothing — it is the one doc that outlives a rewrite |
...
- `docs/ARCHITECTURE.md` — the umbrella: **the positioning, the three differentiators and the non-goals (§1), plus the six findings the roadmap's decisions rest on (§1.1)** — ask §1 before adding a feature — then topology, seams, control-flow diagrams, the function map, the design-decision index (`D#`), the implementation-level known constraints (§26) and the bash 3.2 portability contract. Diagrams, flows and decisions only — **detail lives in the per-scope docs below**. **Kept in sync on every PR that touches architecture or a contract.**
...
- `docs/AS-BUILT-contract.md` — the frozen CLI contract (ROADMAP D3): command surfaces, gates, every `-j`/`-J` envelope and error shape, exit codes, the configuration surface, and the add-an-engine checklist. The one doc a third-engine author or a port needs.
- `docs/ROADMAP.md` — **only two things: decisions that must survive a rewrite (each with its reason and its reopen condition), and work not done yet.** No changelog, no landed work, no survey data, and no positioning. D6 puts playlist management, the queue and listening history in scope and binds every later feature to the same hard constraint (a keybinding AND an agent verb); what those three ARE lives in `docs/AS-BUILT-player.md` §9.4/§9.5/§9.6, not here. **Three decisions are recorded NOs worth knowing before proposing them again:** a third engine pair (D9.1 — Ximalaya's search half is unreachable, netease is the measured candidate), `ut-search` (D9.2 — a ninth command and a new envelope shape buy too little at two engines), and **the Go rewrite (D10 — closed entirely, TUI and player alike: the differentiator is the contract and the contract is language-independent, so the only gain left is deleting renderer debt; it reopens only for MCP or single-file distribution).** Every feature must ship an **agent surface** (a verb plus a `-j` envelope) alongside its keybinding — a TUI-only feature is half a feature here.
- `docs/RESEARCH-tui-player.md` — the one survey the roadmap's decisions rest on, and it is **two halves with different credibility, which §1 states before anything else**. **§2–§5 are measured** (2026-08-21/25/27): the six-way name screening compressed to one table (ROADMAP D4), the terminal-player landscape by GitHub API (the non-goal in `ARCHITECTURE.md` §1), what publishing the shell version costs, and how ready it is (ROADMAP D2) — including the shellcheck baseline in §5.1. **§6–§11 are read, not run** (2026-08-29): how other players are actually built along five orthogonal axes (where decoding happens · how long the playing process lives · where site knowledge lives · what can drive it · one extraction or many), the four routes to a playable URL on the domestic side, and the agent surfaces that appeared in 2026. Every figure is dated; **re-run the method before reopening anything it settled, and for the read half treat every claim as a pointer to its source** — §12 lists what it could not establish.
...
- **One name per command, and no second spelling of any of them.** `ut-play`, `yt-search`, `yt-resolve`, `bili-search`, `bili-resolve`, `ut-playlist`, `ut-history`, `uting` are the canonical identity — help text, errors, docs, and the PATH entry itself. The suite ships **no short form** (D4 — the short names are taken on npm/PyPI/crates, and a second official spelling is a second thing to keep in sync). A user wanting one writes their own alias. Never add a second name for an existing command.
