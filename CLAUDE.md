...
| roadmap | `ROADMAP.md` | the decided and the sequenced: the decisions, the phases, and the conditions that would reopen a decision. **Positioning and non-goals are NOT here** — they are as-built (`ARCHITECTURE.md` §1), and so is the analysis behind the decisions (§1.1) | nothing — it is the one doc that outlives a rewrite |
