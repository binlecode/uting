...
tests/contract.sh                                             # …and the live half too: ~49s, 295 checks
...
is the umbrella a newcomer opens first — the why of the whole suite — and five per-scope docs
carry the per-module why, each sized by its measured content (a scope earns its own file only
while its why outgrows a chapter of the umbrella, and folds back when it shrinks — the 2026-08
consolidation measured exactly this and kept the split). The **process** — how a unit of work
...
- `docs/ARCHITECTURE.md` — the umbrella: **positioning, the differentiators and the non-goals, plus the findings the roadmap's decisions rest on** — ask it before adding a feature — then topology, seams, control-flow diagrams, the design decisions grouped by module and interface, the known constraints and the bash 3.2 portability contract. Diagrams, flows and decisions — why and how only. **Kept in sync on every change that touches architecture or a contract.**
- `docs/AS-BUILT-contract.md` — why the CLI contract is shaped the way it is, and the semver boundary: what counts as public API and what does not. The shapes themselves live in `usage()` and are proved by `tests/contract.sh` — this doc explains them, it does not restate them.
