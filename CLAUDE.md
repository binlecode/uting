...
**Status: reference implementation.** Not packaged — no installer, no Homebrew formula, and none planned for the shell version (`docs/ROADMAP.md` D2). Users symlink `uting` (the human surface) and `ut-play`/`yt-search`/`yt-resolve`/`bili-search`/`bili-resolve`/`ut-playlist`/`ut-history` (the agent surface) onto their own PATH. **No short name ships** (`docs/ARCHITECTURE.md` §3.1) — the human face carries the project's own name, so `~/bin/uting` is a plain symlink with the same word at both ends. A user wanting a short form writes their own alias.
...
tests/contract.sh                                             # …and the live half too: ~100s, 278 checks
...
| `shell/yt-resolve` | **The YouTube engine, half two** — handle → `{stream_urls[], http_headers{}, title, duration, format}`, plus `--info`, `--transcript`, `--auth` and `--quality` (the (mode, tier) → format-sort table lives here, like the mode→format table). Owns the PO-token probe, the cookie decision, the mode→format table and the yt-dlp error vocabulary. Every site-specific fact in the suite lives in this file or in `yt-search`; adding a source is adding a pair like it |
...
**Primitives sit behind seams** (`docs/ARCHITECTURE.md` §5), and the seams are now split by file: `mpv` behind `run_mpv()` in `ut-play` (single play seam) plus the `mpv_supports_vo()` capability probe; `yt-dlp` only in the engines (`fetch_results` in `yt-search`; `dump_once`, `resolve_info`, `resolve_transcript` in `yt-resolve`; `dump_once`, `resolve_info` in `bili-resolve`); the Bilibili HTTP seams are `fetch_page_once` in `bili-search` and `fetch_view_once` in `bili-resolve` (`--parts`), the only two places in the suite that build a request by hand; `curl` also backs `probe_raw` in `yt-resolve` (the fetchability probe); `jq` pervasive. Keep new primitive calls inside the existing seams — and a yt-dlp call in `ut-play` or an mpv call in an engine is a layering violation, not a seam.
...
- **An initiative descends from the roadmap; research is its input, not a stage.** Outside-world questions are answered against primary sources and land as `docs/RESEARCH-<topic>.md` with each claim cited, on their own cadence. The `PLAN-` names the roadmap entry it implements, never the research doc.
- **A landing is a checklist, not a feeling.** The unit of work closes against the document that authorized it, atomically: every plan item landed or explicitly deferred, every `done_when` executed rather than read, accepted pre-mortem fixes verified in, review findings resolved, the as-built docs resynced, the `PLAN-` deleted, the version judged. Skipping the housekeeping half means the work has not landed — the code is merely present. Landing closes the unit of work, not the code's liability: that stays with `docs/AS-BUILT-verification.md` §25 and the audit below.
...
Today there are **six** as-built docs, and — like everything else under `docs/` — **all six are
written in Chinese**; this file and the README are the English surface.
`docs/ARCHITECTURE.md` is the umbrella a newcomer opens
first — diagrams, flows, pseudo-code and decisions; five per-scope docs
carry the detail: `AS-BUILT-contract.md` (the frozen CLI surface), `AS-BUILT-engine.md` (the
site half), `AS-BUILT-player.md` (the player, the queue, the two stores),
`AS-BUILT-tui.md` (the human face) and `AS-BUILT-verification.md` (risk register +
verification matrix). The **process** — how a unit of work moves through the repo — is not one
of them: it lives in this file, above. **A moved section keeps its NUMBER**,
so an old citation changes filename and nothing else, and ARCHITECTURE keeps a one-line
tombstone at every number it gave away. A further `AS-BUILT-<scope>.md` splits out when one
earns it. The rule that keeps the family honest is
...
- `docs/ARCHITECTURE.md` — the umbrella: **the positioning, the three differentiators and the non-goals (§1), plus the six findings the roadmap's decisions rest on (§1.1)** — ask §1 before adding a feature — then topology, seams, control-flow diagrams, the function map, the design decisions grouped by module and interface (§3.1–§3.8, cited by subsection — ARCHITECTURE carries no `D#` of its own), the implementation-level known constraints (§26) and the bash 3.2 portability contract. Diagrams, flows and decisions only — **detail lives in the per-scope docs below**. **Kept in sync on every PR that touches architecture or a contract.**
...
- `docs/AS-BUILT-contract.md` — the frozen CLI contract (ARCHITECTURE §3.8): command surfaces, gates, every `-j`/`-J` envelope and error shape, exit codes, the configuration surface, and the add-an-engine checklist. The one doc a third-engine author or a port needs.
- `docs/ROADMAP.md` — **only what is still open: recorded NOs (each with its reason and its reopen condition), reopen triggers for settled decisions, and work not done yet.** No changelog, no landed work, no survey data, and no positioning — landed decisions live in `docs/ARCHITECTURE.md` §3.1–§3.8, grouped by module and interface. **Its three entries are recorded NOs worth knowing before proposing them again:** packaging/distribution (D2 — reference implementation, no installer, no `v*` tags), a third engine pair (D9.1 — Ximalaya's search half is unreachable, netease is the measured candidate) and `ut-search` (D9.2 — a ninth command and a new envelope shape buy too little at two engines), and **the Go rewrite (D10 — closed entirely, TUI and player alike: the differentiator is the contract and the contract is language-independent, so the only gain left is deleting renderer debt; it reopens only for MCP or single-file distribution).** Every feature must ship an **agent surface** (a verb plus a `-j` envelope) alongside its keybinding (`docs/ARCHITECTURE.md` §3.5) — a TUI-only feature is half a feature here.
- `docs/RESEARCH-tui-player.md` — the one survey the suite's recorded decisions rest on, and it is **two halves with different credibility, which §1 states before anything else**. **§2–§5 are measured** (2026-08-21/25/27): the six-way name screening compressed to one table (ARCHITECTURE §3.1), the terminal-player landscape by GitHub API (the non-goal in `ARCHITECTURE.md` §1), what publishing the shell version costs, and how ready it is (ROADMAP D2) — including the shellcheck baseline in §5.1. **§6–§11 are read, not run** (2026-08-29): how other players are actually built along five orthogonal axes (where decoding happens · how long the playing process lives · where site knowledge lives · what can drive it · one extraction or many), the four routes to a playable URL on the domestic side, and the agent surfaces that appeared in 2026. Every figure is dated; **re-run the method before reopening anything it settled, and for the read half treat every claim as a pointer to its source** — §12 lists what it could not establish.
...
- **One name per command, and no second spelling of any of them.** `ut-play`, `yt-search`, `yt-resolve`, `bili-search`, `bili-resolve`, `ut-playlist`, `ut-history`, `uting` are the canonical identity — help text, errors, docs, and the PATH entry itself. The suite ships **no short form** (ARCHITECTURE §3.1 — the short names are taken on npm/PyPI/crates, and a second official spelling is a second thing to keep in sync). A user wanting one writes their own alias. Never add a second name for an existing command.
- Per-request choices are **flags**; set-once tuning is a **configuration key** — suite-wide `UT_*`, per-engine `YT_*` / `BILI_*` (the prefix is also what makes a name config-reachable, so a constant must not wear one). It resolves through four levels — flag > environment > the user's config file > the shipped `config` — and its default is declared once, in `config`, never behind a `:-` in a script. Keys and shapes: `docs/AS-BUILT-contract.md` §5. This keeps each verb's flag surface narrow enough for a small model to call safely; do not add a flag for something a user sets once.
