...
| `shell/yt-resolve` | **The YouTube engine, half two** — handle → `{stream_urls[], http_headers{}, title, duration, format}`, plus `--info`, `--transcript` and `--auth`. Owns the PO-token probe, the cookie decision, the mode→format table and the yt-dlp error vocabulary. Every site-specific fact in the suite lives in this file or in `yt-search`; adding a source is adding a pair like it |
...
| `shell/bili-resolve` | **The Bilibili engine, half two** — handle → the same resolve envelope, over `yt-dlp`. No request signing, no stream selection, no CDN logic: that is a thousand lines to redo what the dependency maintains. Owns the BV/av handle shapes, the cookie decision, the mode→format table; `--info` and `--auth` are here too. No `--transcript` — the site has no captions, and an engine states a missing capability by not having the verb |
...
| `shell/uting` | The human face — self-rendered list and focus card, live filter, reflowing pagination, three playback states, en/zh chrome, ASCII fallback, themes. **Pure orchestration: zero YouTube logic.** No TUI framework, no fzf |
